...
export class Physics {
    ...
    _calcCollisionDamage(attacker, defender, baseDamage, nx, nz) {
        ...
        const chargeMult = 1 + (chargeSpeed * 0.03);
        damage *= chargeMult;
    ...
    resolveCollision(a, b, nx, nz, velAlongNormal) {
        ...
        if (aIsPlayer) a.takeDamage(resultB.damage, 0.25);
        else a.takeDamage(resultB.damage);

        if (bIsPlayer) b.takeDamage(resultA.damage, 0.25);
        else b.takeDamage(resultA.damage);
        ...
        if (this.particles && a.rpm > 0 && b.rpm > 0) {
    ...
    update(dt) {
        ...
        if (pDist > pEffRadius) {
            ...
            const baseWallDmg = isCharging ? (20 + speed * 3) : 8;
            const wallDamage = baseWallDmg * dt * (50 / (50 + this.player.stats.def));

            this.player.takeDamage(wallDamage, 0); // walls don't break parts

            if (this.audio && this.player.rpm > 0) this.audio.startGrind();
        ...
        if (this.enemies?.list) {
            for (let i = 0; i < this.enemies.list.length; i++) {
                ...
                if (!enemy || !enemy.position || enemy.rpm <= 0) continue;
